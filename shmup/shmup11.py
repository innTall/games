...
# загрузка кадров анимации для визуализации взрывов
explosion_anim = {}
explosion_anim['lg'] = []
explosion_anim['sm'] = []
...
for i in range(9):
  ...
  filename = 'sonExpl{}.png'.format(i)
  img = pygame.image.load(path.join(img_dir, filename)).convert()
  img.set_colorkey(BLACK)
  explosion_anim['player'].append(img)

# создание миниатюрных изображений игрока
player_img = pygame.image.load(path.join(img_dir, "playerShip1_orange.png")).convert()
...
class Player(pygame.sprite.Sprite):
  def __init__(self):
    ...
    self.lives = 3 # счетчик жизней
    self.hidden = False # показывать / скрывать игрока
    self.hide_timer = pygame.time.get_ticks() # таймер сокрытия игрока
  ...
  def hide(self): # достаточно убрать игрока с нижней части на короткое время
    self.hidden = True
...
# Отображение счетчика жизней - числовое
...
# создание спрайта взрыва на экране
...
while running:
  ...
  for hit in hits:
    ...
    newmob()
    if player.shield <= 0:
      death_explosion = Explosion(player.rect.center, 'player') # смерть игрока
      all_sprites.add(death_explosion)
      # player.kill() # не удалять игрока до завершения анимации
      player.hide() # сокрытие игрока после смерти
      player.lives -= 1 # отнимается одна жизнь
      player.shield = 100 # счетчик здоровья сбрасывается
