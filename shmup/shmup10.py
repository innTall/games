...
class Player(pygame.sprite.Sprite):
  def __init__(self):
    ...
    self.shield = 100
    self.radius = 20
    self.shoot_delay = 250 # время между появлением пуль (в миллисекундах)
    self.last_shot = pygame.time.get_ticks() # время от последней пули
...
while running:
  ...
  for hit in hits:
    ...
    expl = Explosion(hit.rect.center, 'sm') # появление спрайта взрыва
  ...
  for hit in hits:
    ...
    expl = Explosion(hit.rect.center, 'lg') # появление спрайта взрыва
    all_sprites.add(expl)
    newmob()
