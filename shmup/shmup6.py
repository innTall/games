...
meteor_images = [] # для задания размеров астероидов случайным образом
...
class Mob(pygame.sprite.Sprite):
  def __init__(self):
    pygame.sprite.Sprite.__init__(self)
    self.image_orig = random.choice(meteor_images) # случайный выбор
    self.image_orig.set_colorkey(BLACK)
    self.image = self.image_orig.copy() # копирование для картинки вращения
    self.rect = self.image.get_rect()
    ...
    self.rot_speed = random.randrange(-8, 8) # сколько градусов за раз
    self.last_update = pygame.time.get_ticks() # частота изменений
    pygame.draw.circle(self.image, RED, self.rect.center, self.radius)

  # # вращение спрайтов
  def rotate(self):
    now = pygame.time.get_ticks() # проверка времени для очередного обновления
    if now - self.last_update > 50: # обновление
      self.last_update = now # можно делать вращение
    self.rot = (self.rot + self.rot_speed) % 360 # общая степень вращения
    new_image = pygame.transform.rotate(self.image_orig, self.rot)
    old_center = self.rect.center # положение центра прямоугольника
    self.image = new_image # 
    self.rect = self.image.get_rect() # вычислить новый центр
    self.rect.center = old_center # сохранить координаты в новый прямоугольник
